...
public class PushStream {
    ...
    public PushStream(Context context, String taskId, long userId) {
        this.context = context;
        this.taskId = taskId;
        this.userIds.add(userId);
...
//        layout.height = ScreenUtils.getScreenHeight(); // 在主画面的显示高度
    ...
    private ArrayList<NERtcLiveStreamUserTranscoding> createUserTranscodingList(List<Long> userIds, Rect[] rectUsers) {
        ...
        for (int i = 0; i < userIds.size() && i < rectUsers.length; i++) {
            ...
            if (i == 0) {
                userTranscoding.width = rectLayout.width(); // 在主画面的显示宽度
                userTranscoding.height = rectLayout.height(); // 在主画面的显示高度
            } else {
                if (isOneToMore()) {
                    userTranscoding.videoPush = false; // 推送该用户视频流
                }
                userTranscoding.width = 200; // 在主画面的显示宽度
                userTranscoding.height = 200; // 在主画面的显示高度
                userTranscoding.x = 30; // 离主画面左边距
                userTranscoding.y = 30; // 离主画面上边距
            }
...
//            userTranscoding.width = rectLayout.width(); // 在主画面的显示宽度
//            userTranscoding.height = rectLayout.height(); // 在主画面的显示高度
//            userTranscoding.width = width; // 在主画面的显示宽度
//            userTranscoding.height = height; // 在主画面的显示高度
