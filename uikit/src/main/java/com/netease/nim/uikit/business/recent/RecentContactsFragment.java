...
public class RecentContactsFragment extends TFragment {
    ...
    private void showLongClickMenu(final RecentContact recent, final int position) {
        ...
        alertDialog.addItem(title, () -> {
            ...
            bundle2.putString("showContent", "是否删除该聊天？");
            ...
            baseTipsDialog1.setOnConfirmListener(new BaseTipsDialog.OnBaseTipsListener() {
                ...
                public void comfirm() {
                    ...
                    if (StickTopCache.isStickTop(recent)) {
        ...
        // 删漫游
        title = getString(R.string.delete_roaming_msg);
        alertDialog.addItem(title, () -> {
        ...
        title = "单向删除最后两条消息";
        alertDialog.addItem(title, () -> {
            List<IMMessage> msgList = msgService.queryMessageListExBlock(
                    MessageBuilder.createEmptyMessage(sessionId, sessionType, TimeUtil.currentTimeMillis()),
                    QueryDirectionEnum.QUERY_OLD, 2, false
            );
            if (!CollectionUtil.isEmpty(msgList)) {
                msgService.deleteMsgSelf(msgList, "单向删除最后两条消息")
                        .setCallback(new RequestCallback<Long>() {
                            @Override
...
//        // 删除本地会话，不清除本地消息和未读记录
//        title = getString(R.string.delete_chat_only);
//        alertDialog.addItem(title, () -> {
//            msgService.deleteRecentContact(sessionId, sessionType, DeleteTypeEnum.LOCAL, false)
//                    .setCallback(deleteRecentCallback);
//            adapter.remove(position);
//            postRunnable(() -> refreshMessages(true));
//        });
//
//        // 删除本地会话，并视为已读
//        title = getString(R.string.delete_chat_and_mark_read);
//        alertDialog.addItem(title, () -> {
